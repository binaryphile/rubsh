...
be stored in hashes.  The variable names are the id, prefixed by
double-underscore.
...
used by ruby, so the key is gotten by appending the variable name to the
objectid without having to add the "@" since it's already in the
variable name.
